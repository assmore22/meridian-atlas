...
async function sdk() {
  ...
  return { createClient: mod.createClient, createAccount: mod.createAccount, testnetBradbury: chains.testnetBradbury };
}

let readClient: any = null;

export async function getReadClient(): Promise<any> {
  const { createClient, createAccount, testnetBradbury } = await sdk();
  if (!readClient) {
    readClient = createClient({ chain: testnetBradbury, account: createAccount() });
  }
  return readClient;
}

async function ensureBradbury(provider: EthProvider) {
  try {
...
// Bradbury uses normal testnet gas; keep wallet fee fields intact and only provide a gas fallback.
...
export async function connectWallet(): Promise<string> {
  ...
  await ensureBradbury(window.ethereum);
...
export async function writeContract(
  ...
  const { createClient, testnetBradbury } = await sdk();
  const provider = patchProvider(window.ethereum);
  const client: any = createClient({ chain: testnetBradbury, account: signer, provider });
